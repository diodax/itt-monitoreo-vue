          ...
          <form class="form-horizontal" name="form" @submit.prevent="submitForm()">
            <div class="box-body">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-8">
                  <p>{{ model.doctor.firstName + ' ' + model.doctor.lastName + ' (' + model.doctor.username + ')' }}</p>
                  <!-- <select class="form-control" id="doctor" name="doctor" v-model="model.doctor" v-validate="'required'" disabled>
              ...
              <div class="form-group">
                ...
                <div class="col-sm-8">
                  <p>{{ model.patient.firstName + ' ' + model.patient.lastName + ' (' + model.patient.username + ')' }}</p>
                  <!-- <select class="form-control" id="patient" name="patient" v-model="model.patient" v-validate="'required'" disabled>
            ...
            <div class="box-footer">
              <div class="margin pull-right">
                <router-link :to="{ path: '/dashboard' }" class="btn btn-default" active-class=" ">Go Back</router-link>
              </div>
            </div>
          </form>
        </div>
...
export default {
  ...
  created() {
    let self = this;
    api.get('/prescription/' + self.id + '?populate=[doctor,patient]').then(function(response) {
