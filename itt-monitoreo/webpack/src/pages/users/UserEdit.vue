...
export default {
  ...
  created() {
    let self = this;
    api.get('/user/' + self.id + '?populate=[roles]').then(function(response) {
        self.model = response.data;
        self.model.birthDate = moment(response.data.birthDate).format('YYYY-MM-DD');
        self.selectedRoles = response.data.roles.map(function(element) {
          return {
            value: element.id,
            label: element.name
          };
  ...
  methods: {
    submitForm() {
      ...
      api.put('/user/' + self.id, self.model).then(function(response) {
